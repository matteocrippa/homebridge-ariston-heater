...
**🔄 Major KISS Refactor - Rate Limiting Fix:**

This release completely rewrites the API interaction layer to prevent rate limiting issues with the Ariston API.

**🚀 Key Changes:**
- **Separated discovery from data fetching** - Variant discovery now happens once at startup and is cached permanently [`discovery-split`]
- **Removed refreshOnGet** - HomeKit characteristic reads no longer trigger API calls, preventing excessive requests [`no-refresh-on-get`]
- **Simple single-call data fetching** - `getPlantData()` makes exactly 1 API call per refresh [`single-call`]
- **1-second delay between discovery attempts** - Prevents rate limiting during initial variant detection [`discovery-delay`]
- **Progressive backoff on failures** - Waits `30 × consecutiveFailures` seconds (max 5 min) before retrying [`backoff`]
- **Rediscovery only after 5 consecutive failures** - Not on every error [`smart-rediscovery`]

**🗑️ Removed (Simplification):**
- Removed Eve custom characteristics (weren't visible in Apple Home anyway)
- Removed `refreshOnGet` and `refreshOnGetCooldown` config options
- Removed complex staleness checking logic
- Removed `RateLimitError` class (simplified error handling)

**⚙️ Configuration Changes:**
- `pollInterval` minimum changed from 30 minutes to 5 minutes (default still 30 min)
- `minTemp` default changed from 35 to 40
- `maxTemp` default changed from 70 to 65
- Removed: `eveCharacteristics`, `refreshOnGet`, `refreshOnGetCooldownSeconds`

**📊 API Call Reduction:**
| Scenario | Before | After |
|----------|--------|-------|
| Startup (cached variant) | 1-2 calls | 2 calls |
| Startup (discovery) | 5-10 calls | 2-6 calls (with delays) |
| Per refresh cycle | 1-5 calls | 1 call |
| HomeKit read | 0-1 calls | 0 calls |

**🛡️ Reliability:**
- 30-second HTTP timeout (was 15s) for slow connections
- Automatic re-login on 401 errors
- Cached values served during API outages
- Clean initialization retry after 60 seconds on failure

**⚠️ Breaking Changes:**
- Eve app custom characteristics removed
- Some config options removed (see above)
...
**🐛 Bug Fix:**
- Fixed target temperature showing wrong value - now uses `reqTemp` (user-set temperature) instead of `procReqTemp` (processed/scheduled temperature) [`temp-fix`]

#### [0.2.5](https://github.com/matteocrippa/homebridge-ariston-heater/compare/0.2.4...0.2.5)

> 1 December 2025

**🛡️ API Resilience Improvements:**
- Fixed cache clearing on API failures - cached variant now preserved during server outages [`cache-preserve`]
- Added detection for Ariston API server issues (500 errors, timeouts) [`server-detection`]
- Improved error messages: now distinguishes between server issues vs device offline [`error-clarity`]
- Cache timestamp refreshes on successful fetch, preventing unnecessary re-discovery [`cache-refresh`]

**🔧 Better Error Handling:**
- New error type: "API server issues (X server errors, Y timeouts)" [`error-types`]
- Longer retry delay (30s) for transient server failures vs immediate retry [`retry-delay`]
- Logs "Using cached data while API is unavailable" during outages [`cache-fallback`]
- Stale cache is now tried first before full re-discovery [`stale-first`]

**📊 Reliability:**
- Plugin continues working with cached data when Ariston servers are down [`offline-mode`]
- Commands (temperature, power) work reliably with preserved variant info [`commands-fix`]
- Reduced unnecessary full variant discovery cycles [`discovery-optimize`]

**✨ Technical:**
- All smoke tests passing [`tests-pass`]
